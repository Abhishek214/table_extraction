...
def write_json(regions: list[TableRegion], path: str | Path, doc_id: str) -> None:
    """Dump tables to a single JSON file."""
    path = Path(path)
...
def write_csv(regions: list[TableRegion], output_dir: str | Path, doc_id: str) -> list[Path]:
    ...
    paths: list[Path] = []
    for i, r in enumerate(regions):
        safe = re.sub(r"[^\w\-_. ]+", "_", r.title or f"table_{i + 1}")
        safe = re.sub(r"\s+", "_", safe).strip("_.")
        safe = safe[:50] or "untitled"
...
def write_excel(regions: list[TableRegion], path: str | Path, doc_id: str) -> None:
    """Write all tables to one Excel workbook, one sheet per table."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
